...
namespace ordertest
{
    class OrderDetails
    {
        static Order Info1 = new Order(2018001, "apple", "Wang", 101011);
        static Order Info2 = new Order(2018002, "melon", "Chen", 205025);
        static Order Info3 = new Order(2018003, "groups", "Shen", 563442);
        List<Order> OrderList = new List<Order>() { Info1, Info2, Info3 };//建立一个由order类对象组成的list


        public void SearchByGoodsName()//按照订单商品名称查找相应订单
        {
        ...
        {
            var m = from n in OrderList where n.OrderNum == 2018001 select n;
            foreach (var n in m)
            {
                Console.WriteLine(n);
            }
        }
        public void SearchByOrderamount()
        {
            var m = from n in OrderList where n.Orderamount > 10000 select n;
            foreach (var n in m)
            {
                Console.WriteLine(n);
            }
